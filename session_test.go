...
var _ = Describe("Session", func() {
	...
	Context("frame handling", func() {
		...
		Context("when handling STREAM frames", func() {
			BeforeEach(func() {
				sess.streamsMap.UpdateTransportParameters(&handshake.TransportParameters{MaxStreams: 100})
			})
		...
		It("handles CONNECTION_CLOSE frames", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			sess.streamsMap.Range(func(s streamI) { s.(*mocks.MockStreamI).EXPECT().Cancel(gomock.Any()) })
	...
	It("process transport parameters received from the peer", func() {
		paramsReceived := make(chan *handshake.TransportParameters)
		sess.paramsReceived = paramsReceived
		_, err := sess.GetOrOpenStream(5)
		Expect(err).ToNot(HaveOccurred())
		go sess.run()
